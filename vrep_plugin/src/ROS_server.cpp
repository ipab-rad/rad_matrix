...
ros::Publisher
ROS_server::infoPublisher; // special publisher that is active also when simulation is not running!
...
struct SDepthCloudPublisherData : public SPointCloudPublisherData {
	...
	SDepthCloudPublisherData(const std::string& obj_name) :
		SPointCloudPublisherData(obj_name) {
...
bool ROS_server::initialize() {
	...
	// ROS_INFO_STREAM("ENABLING ALL SERVICES on node: " <<
	//                 "vrep_" + std::to_string(getpid()));
	enableAPIServices();
	infoPublisher = node->advertise<vrep_common::VrepInfo>("info",
	                                                       1); // special case! This is the only publisher always active!
...
std::string ROS_server::addPublisher(const char* topicName, int queueSize,
                                     int streamCmd, int auxInt1, int auxInt2, const char* auxString,
                                     int publishCnt) {
	// 0. Check if the streamCmd is valid:
	if (streamCmd == simros_strmcmdnull_start)
		return ("");
	if ( (streamCmd >= simros_strmcmdnull_subscriber_start) &&
	        (streamCmd <= simros_strmcmdint_start) )
		return ("");
	if ( (streamCmd >= simros_strmcmdint_subscriber_start) &&
	        (streamCmd <= simros_strmcmdintint_start) )
		return ("");
	if ( (streamCmd >= simros_strmcmdintint_subscriber_start) &&
	        (streamCmd <= simros_strmcmdstring_start) )
		return ("");
	if ( (streamCmd >= simros_strmcmdstring_subscriber_start) &&
	        (streamCmd <= simros_strmcmdintstring_start) )
		return ("");
...
int ROS_server::getPublisherIndexFromCmd(int streamCmd, int auxInt1,
                                         int auxInt2, const char* auxString) {
	for (unsigned int i = 0; i < publishers.size(); i++) {
		if (publishers[i].cmdID == streamCmd) {
			...
			if ((streamCmd < simros_strmcmdreserved_start) &&
				...
				if ( (publishers[i].auxInt1 == auxInt1) &&
				        (publishers[i].auxStr.compare(auxString) == 0) )
					return (i); // Yes!
			}
		}
	}
...
int ROS_server::removePublisher(const char* topicName,
                                bool ignoreReferenceCounter) {
	for (unsigned int i = 0; i < publishers.size(); i++) {
		if (publishers[i].topicName.compare(topicName) == 0) {
...
bool ROS_server::launchPublisher(SPublisherData& pub, int queueSize) {
	if (pub.cmdID == simros_strmcmd_get_odom_data) {
		...
		pub.generalPublisher = node->advertise<nav_msgs::Odometry>(pub.topicName,
		                                                           queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	if (pub.cmdID == simros_strmcmd_get_laser_scanner_data) {
		// courtesy of George Moustris
		pub.generalPublisher = node->advertise<sensor_msgs::LaserScan>(pub.topicName,
		                                                               queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_get_joint_state) {
		if ( (pub.auxInt1 != sim_handle_all) &&
		        (simGetObjectType(pub.auxInt1) != sim_object_joint_type) )
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<sensor_msgs::JointState>(pub.topicName,
		                                                                queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_get_array_parameter) {
		...
		if (simGetArrayParameter(pub.auxInt1, dummy) == -1)
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<geometry_msgs::Point32>(pub.topicName,
		                                                               queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	if (pub.cmdID == simros_strmcmd_get_boolean_parameter) {
		if (simGetBooleanParameter(pub.auxInt1) == -1)
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<std_msgs::UInt8>(pub.topicName,
		                                                        queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	if (pub.cmdID == simros_strmcmd_get_dialog_result) {
		if (simGetDialogResult(pub.auxInt1) == -1)
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<std_msgs::Int32>(pub.topicName,
		                                                        queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_get_floating_parameter) {
		...
		if (simGetFloatingParameter(pub.auxInt1, &dummy) == -1)
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<std_msgs::Float32>(pub.topicName,
		                                                          queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_get_string_signal) {
		pub.generalPublisher = node->advertise<std_msgs::String>(pub.topicName,
		                                                         queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_get_object_group_data) {
		pub.generalPublisher = node->advertise<vrep_common::ObjectGroupData>
		                       (pub.topicName, queueSize);
		pub.dependencyCnt++;
	...
	if (pub.cmdID == simros_strmcmd_get_object_pose) {
		...
		if (simGetObjectPosition(pub.auxInt1, pub.auxInt2, dummy) == -1)
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<geometry_msgs::PoseStamped>
		                       (pub.topicName, queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_get_objects) {
		pub.generalPublisher = node->advertise<std_msgs::Int32MultiArray>(pub.topicName,
		                                                                  queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_get_vision_sensor_depth_buffer) {
		if (simGetObjectType(pub.auxInt1) != sim_object_visionsensor_type)
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<vrep_common::VisionSensorDepthBuff>
		                       (pub.topicName, queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_get_depth_sensor_data) {
		...
		pub.generalPublisher = node->advertise<sensor_msgs::PointCloud2>(pub.topicName,
		                                                                 queueSize);
		pub.dependencyCnt++;
	...
	if (pub.cmdID == simros_strmcmd_get_vision_sensor_info) {
		if (simGetObjectType(pub.auxInt1) != sim_object_visionsensor_type)
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<sensor_msgs::CameraInfo>(pub.topicName,
		                                                                queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_read_force_sensor) {
		if (simReadForceSensor(pub.auxInt1, NULL, NULL) == -1)
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<vrep_common::ForceSensorData>
		                       (pub.topicName, queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_read_proximity_sensor) {
		if (simGetObjectType(pub.auxInt1) != sim_object_proximitysensor_type)
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<vrep_common::ProximitySensorData>
		                       (pub.topicName, queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_read_vision_sensor) {
		if (simGetObjectType(pub.auxInt1) != sim_object_visionsensor_type)
			return (false); // invalid data!
		pub.generalPublisher = node->advertise<vrep_common::VisionSensorData>
		                       (pub.topicName, queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	if (pub.cmdID == simros_strmcmd_get_twist_status) {
		pub.generalPublisher = node->advertise<geometry_msgs::TwistStamped>
		                       (pub.topicName, queueSize);
		pub.dependencyCnt++;
		return (true);
	}
	...
	for (int pubI = 0; pubI < int(publishers.size()); pubI++) {
		...
		if (publishers[pubI].publishCnt != -1) {
			...
			if (publishers[pubI].cmdID == simros_strmcmd_get_vision_sensor_image)
				removeThisPublisher = streamVisionSensorImage(publishers[pubI],
				                                              inf.headerInfo.stamp, publishedSomething);
			...
			if (publishers[pubI].cmdID == simros_strmcmd_get_depth_sensor_data)
				removeThisPublisher = streamDepthSensorCloud(publishers[pubI],
				                                             inf.headerInfo.stamp, publishedSomething);
			...
			if (publishers[pubI].cmdID == simros_strmcmd_get_object_float_parameter) {
				float val;
				if (simGetObjectFloatParameter(publishers[pubI].auxInt1,
				                               publishers[pubI].auxInt2, &val) > 0) {
					std_msgs::Float32 fl;
					fl.data = val;
					publishedSomething = true;
					publishers[pubI].generalPublisher.publish(fl);
			...
			if (publishers[pubI].cmdID == simros_strmcmd_get_object_group_data) {
				...
				if (getObjectGroupData(publishers[pubI].auxInt1, publishers[pubI].auxInt2,
			...
			if (publishers[pubI].cmdID == simros_strmcmd_get_object_int_parameter) {
				int val;
				if (simGetObjectIntParameter(publishers[pubI].auxInt1, publishers[pubI].auxInt2,
				                             &val) > 0) {
					std_msgs::Int32 fl;
					fl.data = val;
					publishedSomething = true;
			...
			if (publishers[pubI].cmdID == simros_strmcmd_get_object_pose) {
				float val[4];
				if (simGetObjectPosition(publishers[pubI].auxInt1, publishers[pubI].auxInt2,
				                         val) != -1) {
			...
			if (publishers[pubI].cmdID == simros_strmcmd_get_ui_button_property) {
				int p = simGetUIButtonProperty(publishers[pubI].auxInt1,
				                               publishers[pubI].auxInt2);
				if (p != -1) {
					std_msgs::Int32 fl;
					fl.data = p;
					publishedSomething = true;
			...
			if (publishers[pubI].cmdID == simros_strmcmd_get_vision_sensor_info) {
				...
				if (simGetVisionSensorResolution(handle, resol) != -1) {
					...
					info.P[0] = info.K[0]; info.P[1] = 0;         info.P[2] = info.K[2];
					info.P[3] = 0;
					info.P[4] = 0;         info.P[5] = info.K[4]; info.P[6] = info.K[5];
					info.P[7] = 0;
					info.P[8] = 0;         info.P[9] = 0;         info.P[10] = 1;
					info.P[11] = 0;
			...
			if (publishers[pubI].cmdID == simros_strmcmd_read_proximity_sensor) {
				...
				int detectedObject;
				float detectedN[3];
				int r = simReadProximitySensor(publishers[pubI].auxInt1, detectedPt,
				                               &detectedObject, detectedN);
				if (r != -1) {
					if (r > 0) {
			...
			if (publishers[pubI].cmdID == simros_strmcmd_read_vision_sensor) {
				float* auxValues;
				int* auxValuesCount;
				int r = simReadVisionSensor(publishers[pubI].auxInt1, &auxValues,
				                            &auxValuesCount);
				if (r >= 0) {
...
bool ROS_server::getObjectGroupData(int objectType, int dataType,
                                    std::vector<int>& handles, std::vector<int>& intData,
                                    std::vector<float>& floatData, std::vector<std::string>& stringData) {
	handles.clear();
	intData.clear();
	floatData.clear();
	stringData.clear();
	...
	if ((objectType == sim_appobj_object_type) ||
	        ((objectType >= sim_object_shape_type) &&
	         (objectType < sim_object_type_end)) ) {
		if (objectType == sim_appobj_object_type)
...
bool ROS_server::streamVisionSensorImage(SPublisherData& pub,
                                         const ros::Time& now, bool& publishedSomething) {
	...
	simReleaseBuffer(objName);
	image_msg.encoding =
	    sensor_msgs::image_encodings::RGB8; //Set the format to be RGB with 8bits per channel
...
bool ROS_server::streamLaserCloud(SPublisherData& pub, const ros::Time& now,
                                  bool& publishedSomething) {
	SPointCloudPublisherData* pcd = (SPointCloudPublisherData*)
	                                pub.specificPublisherData;
	if (!pcd)
		return (false);
	simInt datalen = 0;
	...
	pcd->pointcloud.header.frame_id = objIdToFrameId(
	                                      pub.auxInt1); // Courtesy of Federico Ferri
...
bool ROS_server::streamDepthSensorCloud(SPublisherData& pub,
                                        const ros::Time& now, bool& publishedSomething) {
	SDepthCloudPublisherData* pcd = (SDepthCloudPublisherData*)
	                                pub.specificPublisherData;
	if (!pcd)
		return (false);
	...
	for (unsigned int i = 0; i < datalen; i++) {
		...
		std::copy(raw, raw + 12,
		          pcd->pointcloud.data.begin() + i * pcd->pointcloud.point_step);
		unsigned int p = i * 3;
		uint8_t argb[4] = {image_buf[p + 2], image_buf[p + 1], image_buf[p], 0};
		std::copy(argb, argb + 4,
		          pcd->pointcloud.data.begin() + i * pcd->pointcloud.point_step + 12);
	}
	pcd->pointcloud.header.frame_id = objIdToFrameId(
	                                      pub.auxInt1); // Courtesy of Federico Ferri
...
//========================================= API SUBSCRIPTIONS======================================================
//=================================================================================================================
int ROS_server::addSubscriber(const char* topicName, int queueSize,
                              int streamCmd, int auxInt1, int auxInt2, const char* auxString,
                              int callbackTag_before, int callbackTag_after) {
	// 1. Check if the streamCmd is valid:
	if (streamCmd <= simros_strmcmdnull_subscriber_start)
		return (-1);
	if ( (streamCmd >= simros_strmcmdint_start) &&
	        (streamCmd <= simros_strmcmdint_subscriber_start) )
		return (-1);
	if ( (streamCmd >= simros_strmcmdintint_start) &&
	        (streamCmd <= simros_strmcmdintint_subscriber_start) )
		return (-1);
	if ( (streamCmd >= simros_strmcmdstring_start) &&
	        (streamCmd <= simros_strmcmdstring_subscriber_start) )
		return (-1);
	if ( (streamCmd >= simros_strmcmdintstring_start) &&
	        (streamCmd <= simros_strmcmdintstring_subscriber_start) )
		return (-1);
	if (streamCmd >= simros_strmcmdreserved_start)
		return (-1);
	...
	CSubscriberData* sub = new CSubscriberData(node, nm.c_str(), queueSize,
	                                           streamCmd, auxInt1, auxInt2, auxString, callbackTag_before, callbackTag_after,
	                                           &images_streamer, imgStreamerCnt);
	if (sub->getIsValid()) {
...
void ROS_server::enableAPIServices() {
	simRosAddStatusbarMessageServer =
	    node->advertiseService("simRosAddStatusbarMessage",
	                           ROS_server::simRosAddStatusbarMessageService);
	simRosAuxiliaryConsoleCloseServer =
	    node->advertiseService("simRosAuxiliaryConsoleClose",
	                           ROS_server::simRosAuxiliaryConsoleCloseService);
	simRosAuxiliaryConsoleOpenServer =
	    node->advertiseService("simRosAuxiliaryConsoleOpen",
	                           ROS_server::simRosAuxiliaryConsoleOpenService);
	simRosAuxiliaryConsolePrintServer =
	    node->advertiseService("simRosAuxiliaryConsolePrint",
	                           ROS_server::simRosAuxiliaryConsolePrintService);
	simRosAuxiliaryConsoleShowServer =
	    node->advertiseService("simRosAuxiliaryConsoleShow",
	                           ROS_server::simRosAuxiliaryConsoleShowService);
	simRosBreakForceSensorServer = node->advertiseService("simRosBreakForceSensor",
	                                                      ROS_server::simRosBreakForceSensorService);
	simRosClearFloatSignalServer = node->advertiseService("simRosClearFloatSignal",
	                                                      ROS_server::simRosClearFloatSignalService);
	simRosClearIntegerSignalServer =
	    node->advertiseService("simRosClearIntegerSignal",
	                           ROS_server::simRosClearIntegerSignalService);
	simRosClearStringSignalServer =
	    node->advertiseService("simRosClearStringSignal",
	                           ROS_server::simRosClearStringSignalService);
	simRosCloseSceneServer = node->advertiseService("simRosCloseScene",
	                                                ROS_server::simRosCloseSceneService);
	simRosCopyPasteObjectsServer = node->advertiseService("simRosCopyPasteObjects",
	                                                      ROS_server::simRosCopyPasteObjectsService);
	simRosDisplayDialogServer = node->advertiseService("simRosDisplayDialog",
	                                                   ROS_server::simRosDisplayDialogService);
	simRosEndDialogServer = node->advertiseService("simRosEndDialog",
	                                               ROS_server::simRosEndDialogService);
	simRosEraseFileServer = node->advertiseService("simRosEraseFile",
	                                               ROS_server::simRosEraseFileService);
	simRosGetArrayParameterServer =
	    node->advertiseService("simRosGetArrayParameter",
	                           ROS_server::simRosGetArrayParameterService);
	simRosGetBooleanParameterServer =
	    node->advertiseService("simRosGetBooleanParameter",
	                           ROS_server::simRosGetBooleanParameterService);
	simRosGetCollisionHandleServer =
	    node->advertiseService("simRosGetCollisionHandle",
	                           ROS_server::simRosGetCollisionHandleService);
	simRosGetDialogInputServer = node->advertiseService("simRosGetDialogInput",
	                                                    ROS_server::simRosGetDialogInputService);
	simRosGetDialogResultServer = node->advertiseService("simRosGetDialogResult",
	                                                     ROS_server::simRosGetDialogResultService);
	simRosGetDistanceHandleServer =
	    node->advertiseService("simRosGetDistanceHandle",
	                           ROS_server::simRosGetDistanceHandleService);
	simRosGetFloatingParameterServer =
	    node->advertiseService("simRosGetFloatingParameter",
	                           ROS_server::simRosGetFloatingParameterService);
	simRosGetFloatSignalServer = node->advertiseService("simRosGetFloatSignal",
	                                                    ROS_server::simRosGetFloatSignalService);
	simRosGetIntegerParameterServer =
	    node->advertiseService("simRosGetIntegerParameter",
	                           ROS_server::simRosGetIntegerParameterService);
	simRosGetIntegerSignalServer = node->advertiseService("simRosGetIntegerSignal",
	                                                      ROS_server::simRosGetIntegerSignalService);
	simRosGetJointMatrixServer = node->advertiseService("simRosGetJointMatrix",
	                                                    ROS_server::simRosGetJointMatrixService);
	simRosGetJointStateServer = node->advertiseService("simRosGetJointState",
	                                                   ROS_server::simRosGetJointStateService);
	simRosGetLastErrorsServer = node->advertiseService("simRosGetLastErrors",
	                                                   ROS_server::simRosGetLastErrorsService);
	simRosGetModelPropertyServer = node->advertiseService("simRosGetModelProperty",
	                                                      ROS_server::simRosGetModelPropertyService);
	simRosGetObjectChildServer = node->advertiseService("simRosGetObjectChild",
	                                                    ROS_server::simRosGetObjectChildService);
	simRosGetObjectFloatParameterServer =
	    node->advertiseService("simRosGetObjectFloatParameter",
	                           ROS_server::simRosGetObjectFloatParameterService);
	simRosGetObjectHandleServer = node->advertiseService("simRosGetObjectHandle",
	                                                     ROS_server::simRosGetObjectHandleService);
	simRosGetObjectIntParameterServer =
	    node->advertiseService("simRosGetObjectIntParameter",
	                           ROS_server::simRosGetObjectIntParameterService);
	...
	simRosGetObjectParentServer = node->advertiseService("simRosGetObjectParent",
	                                                     ROS_server::simRosGetObjectParentService);
	//	simRosGetObjectPositionServer = node->advertiseService("simRosGetObjectPosition",ROS_server::simRosGetObjectPositionService);
	simRosGetObjectPoseServer = node->advertiseService("simRosGetObjectPose",
	                                                   ROS_server::simRosGetObjectPoseService);
	simRosGetObjectsServer = node->advertiseService("simRosGetObjects",
	                                                ROS_server::simRosGetObjectsService);
	simRosGetObjectSelectionServer =
	    node->advertiseService("simRosGetObjectSelection",
	                           ROS_server::simRosGetObjectSelectionService);
	simRosGetStringParameterServer =
	    node->advertiseService("simRosGetStringParameter",
	                           ROS_server::simRosGetStringParameterService);
	simRosGetStringSignalServer = node->advertiseService("simRosGetStringSignal",
	                                                     ROS_server::simRosGetStringSignalService);
	simRosGetUIButtonPropertyServer =
	    node->advertiseService("simRosGetUIButtonProperty",
	                           ROS_server::simRosGetUIButtonPropertyService);
	simRosGetUIEventButtonServer = node->advertiseService("simRosGetUIEventButton",
	                                                      ROS_server::simRosGetUIEventButtonService);
	simRosGetUIHandleServer = node->advertiseService("simRosGetUIHandle",
	                                                 ROS_server::simRosGetUIHandleService);
	simRosGetUISliderServer = node->advertiseService("simRosGetUISlider",
	                                                 ROS_server::simRosGetUISliderService);
	simRosGetVisionSensorDepthBufferServer =
	    node->advertiseService("simRosGetVisionSensorDepthBuffer",
	                           ROS_server::simRosGetVisionSensorDepthBufferService);
	simRosGetVisionSensorImageServer =
	    node->advertiseService("simRosGetVisionSensorImage",
	                           ROS_server::simRosGetVisionSensorImageService);
	simRosLoadModelServer = node->advertiseService("simRosLoadModel",
	                                               ROS_server::simRosLoadModelService);
	simRosLoadSceneServer = node->advertiseService("simRosLoadScene",
	                                               ROS_server::simRosLoadSceneService);
	simRosLoadUIServer = node->advertiseService("simRosLoadUI",
	                                            ROS_server::simRosLoadUIService);
	simRosPauseSimulationServer = node->advertiseService("simRosPauseSimulation",
	                                                     ROS_server::simRosPauseSimulationService);
	simRosReadCollisionServer = node->advertiseService("simRosReadCollision",
	                                                   ROS_server::simRosReadCollisionService);
	simRosReadDistanceServer = node->advertiseService("simRosReadDistance",
	                                                  ROS_server::simRosReadDistanceService);
	simRosReadForceSensorServer = node->advertiseService("simRosReadForceSensor",
	                                                     ROS_server::simRosReadForceSensorService);
	simRosReadProximitySensorServer =
	    node->advertiseService("simRosReadProximitySensor",
	                           ROS_server::simRosReadProximitySensorService);
	simRosReadVisionSensorServer = node->advertiseService("simRosReadVisionSensor",
	                                                      ROS_server::simRosReadVisionSensorService);
	simRosRemoveObjectServer = node->advertiseService("simRosRemoveObject",
	                                                  ROS_server::simRosRemoveObjectService);
	simRosRemoveModelServer = node->advertiseService("simRosRemoveModel",
	                                                 ROS_server::simRosRemoveModelService);
	simRosRemoveUIServer = node->advertiseService("simRosRemoveUI",
	                                              ROS_server::simRosRemoveUIService);
	simRosSetArrayParameterServer =
	    node->advertiseService("simRosSetArrayParameter",
	                           ROS_server::simRosSetArrayParameterService);
	simRosSetBooleanParameterServer =
	    node->advertiseService("simRosSetBooleanParameter",
	                           ROS_server::simRosSetBooleanParameterService);
	simRosSetFloatingParameterServer =
	    node->advertiseService("simRosSetFloatingParameter",
	                           ROS_server::simRosSetFloatingParameterService);
	simRosSetFloatSignalServer = node->advertiseService("simRosSetFloatSignal",
	                                                    ROS_server::simRosSetFloatSignalService);
	simRosSetIntegerParameterServer =
	    node->advertiseService("simRosSetIntegerParameter",
	                           ROS_server::simRosSetIntegerParameterService);
	simRosSetIntegerSignalServer = node->advertiseService("simRosSetIntegerSignal",
	                                                      ROS_server::simRosSetIntegerSignalService);
	simRosSetJointForceServer = node->advertiseService("simRosSetJointForce",
	                                                   ROS_server::simRosSetJointForceService);
	simRosSetJointPositionServer = node->advertiseService("simRosSetJointPosition",
	                                                      ROS_server::simRosSetJointPositionService);
	simRosSetJointTargetPositionServer =
	    node->advertiseService("simRosSetJointTargetPosition",
	                           ROS_server::simRosSetJointTargetPositionService);
	simRosSetJointTargetVelocityServer =
	    node->advertiseService("simRosSetJointTargetVelocity",
	                           ROS_server::simRosSetJointTargetVelocityService);
	simRosSetModelPropertyServer = node->advertiseService("simRosSetModelProperty",
	                                                      ROS_server::simRosSetModelPropertyService);
	simRosSetObjectFloatParameterServer =
	    node->advertiseService("simRosSetObjectFloatParameter",
	                           ROS_server::simRosSetObjectFloatParameterService);
	simRosSetObjectIntParameterServer =
	    node->advertiseService("simRosSetObjectIntParameter",
	                           ROS_server::simRosSetObjectIntParameterService);
	simRosSetObjectPoseServer = node->advertiseService("simRosSetObjectPose",
	                                                   ROS_server::simRosSetObjectPoseService);
	simRosSetObjectParentServer = node->advertiseService("simRosSetObjectParent",
	                                                     ROS_server::simRosSetObjectParentService);
	simRosSetObjectPositionServer =
	    node->advertiseService("simRosSetObjectPosition",
	                           ROS_server::simRosSetObjectPositionService);
	simRosSetObjectSelectionServer =
	    node->advertiseService("simRosSetObjectSelection",
	                           ROS_server::simRosSetObjectSelectionService);
	simRosGetInfoServer = node->advertiseService("simRosGetInfo",
	                                             ROS_server::simRosGetInfoService);
	simRosSetSphericalJointMatrixServer =
	    node->advertiseService("simRosSetSphericalJointMatrix",
	                           ROS_server::simRosSetSphericalJointMatrixService);
	simRosSetStringSignalServer = node->advertiseService("simRosSetStringSignal",
	                                                     ROS_server::simRosSetStringSignalService);
	simRosSetUIButtonLabelServer = node->advertiseService("simRosSetUIButtonLabel",
	                                                      ROS_server::simRosSetUIButtonLabelService);
	simRosSetUIButtonPropertyServer =
	    node->advertiseService("simRosSetUIButtonProperty",
	                           ROS_server::simRosSetUIButtonPropertyService);
	simRosSetUISliderServer = node->advertiseService("simRosSetUISlider",
	                                                 ROS_server::simRosSetUISliderService);
	simRosSetVisionSensorImageServer =
	    node->advertiseService("simRosSetVisionSensorImage",
	                           ROS_server::simRosSetVisionSensorImageService);
	simRosStartSimulationServer = node->advertiseService("simRosStartSimulation",
	                                                     ROS_server::simRosStartSimulationService);
	simRosStopSimulationServer = node->advertiseService("simRosStopSimulation",
	                                                    ROS_server::simRosStopSimulationService);
	simRosSynchronousServer = node->advertiseService("simRosSynchronous",
	                                                 ROS_server::simRosSynchronousService);
	simRosSynchronousTriggerServer =
	    node->advertiseService("simRosSynchronousTrigger",
	                           ROS_server::simRosSynchronousTriggerService);
	simRosTransferFileServer = node->advertiseService("simRosTransferFile",
	                                                  ROS_server::simRosTransferFileService);
	simRosEnablePublisherServer = node->advertiseService("simRosEnablePublisher",
	                                                     ROS_server::simRosEnablePublisherService);
	simRosDisablePublisherServer = node->advertiseService("simRosDisablePublisher",
	                                                      ROS_server::simRosDisablePublisherService);
	...
	simRosSetObjectQuaternionServer =
	    node->advertiseService("simRosSetObjectQuaternion",
	                           ROS_server::simRosSetObjectQuaternionService);
	simRosEnableSubscriberServer = node->advertiseService("simRosEnableSubscriber",
	                                                      ROS_server::simRosEnableSubscriberService);
	simRosDisableSubscriberServer =
	    node->advertiseService("simRosDisableSubscriber",
	                           ROS_server::simRosDisableSubscriberService);
	simRosSetJointStateServer = node->advertiseService("simRosSetJointState",
	                                                   ROS_server::simRosSetJointStateService);
	simRosCreateDummyServer = node->advertiseService("simRosCreateDummy",
	                                                 ROS_server::simRosCreateDummyService);
	simRosGetAndClearStringSignalServer =
	    node->advertiseService("simRosGetAndClearStringSignal",
	                           ROS_server::simRosGetAndClearStringSignalService);
	simRosGetObjectGroupDataServer =
	    node->advertiseService("simRosGetObjectGroupData",
	                           ROS_server::simRosGetObjectGroupDataService);
}
...
bool ROS_server::simRosAddStatusbarMessageService(
    vrep_common::simRosAddStatusbarMessage::Request& req,
    vrep_common::simRosAddStatusbarMessage::Response& res) {
...
bool ROS_server::simRosAuxiliaryConsoleCloseService(
    vrep_common::simRosAuxiliaryConsoleClose::Request& req,
    vrep_common::simRosAuxiliaryConsoleClose::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosAuxiliaryConsoleOpenService(
    vrep_common::simRosAuxiliaryConsoleOpen::Request& req,
    vrep_common::simRosAuxiliaryConsoleOpen::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	int* pos = NULL;
	int* siz = NULL;
	float* textCol = NULL;
	...
	res.consoleHandle = simAuxiliaryConsoleOpen(req.title.c_str(), req.maxLines,
	                                            req.mode, pos, siz, textCol, backCol);
	_handleServiceErrors_end(errorModeSaved);
	return true;
}

bool ROS_server::simRosAuxiliaryConsolePrintService(
    vrep_common::simRosAuxiliaryConsolePrint::Request& req,
    vrep_common::simRosAuxiliaryConsolePrint::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosAuxiliaryConsoleShowService(
    vrep_common::simRosAuxiliaryConsoleShow::Request& req,
    vrep_common::simRosAuxiliaryConsoleShow::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosBreakForceSensorService(
    vrep_common::simRosBreakForceSensor::Request& req,
    vrep_common::simRosBreakForceSensor::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosClearFloatSignalService(
    vrep_common::simRosClearFloatSignal::Request& req,
    vrep_common::simRosClearFloatSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosClearIntegerSignalService(
    vrep_common::simRosClearIntegerSignal::Request& req,
    vrep_common::simRosClearIntegerSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	if (req.signalName.length() == 0)
...
bool ROS_server::simRosClearStringSignalService(
    vrep_common::simRosClearStringSignal::Request& req,
    vrep_common::simRosClearStringSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	if (req.signalName.length() == 0)
...
bool ROS_server::simRosCloseSceneService(vrep_common::simRosCloseScene::Request&
                                         req, vrep_common::simRosCloseScene::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosCopyPasteObjectsService(
    vrep_common::simRosCopyPasteObjects::Request& req,
    vrep_common::simRosCopyPasteObjects::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosDisplayDialogService(
    vrep_common::simRosDisplayDialog::Request& req,
    vrep_common::simRosDisplayDialog::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	float* col1 = NULL;
	float* col2 = NULL;
	...
	int uiHandle;
	int handle = simDisplayDialog(req.titleText.c_str(), req.mainText.c_str(),
	                              req.dialogType, req.initialText.c_str(), col1, col2, &uiHandle);
...
bool ROS_server::simRosEndDialogService(vrep_common::simRosEndDialog::Request&
                                        req, vrep_common::simRosEndDialog::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosEraseFileService(vrep_common::simRosEraseFile::Request&
                                        req, vrep_common::simRosEraseFile::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	res.result = -1;
...
bool ROS_server::simRosGetArrayParameterService(
    vrep_common::simRosGetArrayParameter::Request& req,
    vrep_common::simRosGetArrayParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	float vals[3];
...
bool ROS_server::simRosGetBooleanParameterService(
    vrep_common::simRosGetBooleanParameter::Request& req,
    vrep_common::simRosGetBooleanParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetCollisionHandleService(
    vrep_common::simRosGetCollisionHandle::Request& req,
    vrep_common::simRosGetCollisionHandle::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetDialogInputService(
    vrep_common::simRosGetDialogInput::Request& req,
    vrep_common::simRosGetDialogInput::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetDialogResultService(
    vrep_common::simRosGetDialogResult::Request& req,
    vrep_common::simRosGetDialogResult::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetDistanceHandleService(
    vrep_common::simRosGetDistanceHandle::Request& req,
    vrep_common::simRosGetDistanceHandle::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetFloatingParameterService(
    vrep_common::simRosGetFloatingParameter::Request& req,
    vrep_common::simRosGetFloatingParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	float p;
...
bool ROS_server::simRosGetFloatSignalService(
    vrep_common::simRosGetFloatSignal::Request& req,
    vrep_common::simRosGetFloatSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	float signalValue;
...
bool ROS_server::simRosGetIntegerParameterService(
    vrep_common::simRosGetIntegerParameter::Request& req,
    vrep_common::simRosGetIntegerParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	int p;
...
bool ROS_server::simRosGetIntegerSignalService(
    vrep_common::simRosGetIntegerSignal::Request& req,
    vrep_common::simRosGetIntegerSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	int signalValue;
...
bool ROS_server::simRosGetJointMatrixService(
    vrep_common::simRosGetJointMatrix::Request& req,
    vrep_common::simRosGetJointMatrix::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	float v;
...
bool ROS_server::simRosGetJointStateService(
    vrep_common::simRosGetJointState::Request& req,
    vrep_common::simRosGetJointState::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetLastErrorsService(
    vrep_common::simRosGetLastErrors::Request& req,
    vrep_common::simRosGetLastErrors::Response& res) {
...
bool ROS_server::simRosGetModelPropertyService(
    vrep_common::simRosGetModelProperty::Request& req,
    vrep_common::simRosGetModelProperty::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetObjectChildService(
    vrep_common::simRosGetObjectChild::Request& req,
    vrep_common::simRosGetObjectChild::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetObjectFloatParameterService(
    vrep_common::simRosGetObjectFloatParameter::Request& req,
    vrep_common::simRosGetObjectFloatParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	res.result = simGetObjectFloatParameter(req.handle, req.parameterID,
	                                        &res.parameterValue);
	_handleServiceErrors_end(errorModeSaved);
	return true;
}

bool ROS_server::simRosGetObjectHandleService(
    vrep_common::simRosGetObjectHandle::Request& req,
    vrep_common::simRosGetObjectHandle::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetObjectIntParameterService(
    vrep_common::simRosGetObjectIntParameter::Request& req,
    vrep_common::simRosGetObjectIntParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	res.result = simGetObjectIntParameter(req.handle, req.parameterID,
	                                      &res.parameterValue);
	_handleServiceErrors_end(errorModeSaved);
	return true;
}
...
*/
bool ROS_server::simRosGetObjectParentService(
    vrep_common::simRosGetObjectParent::Request& req,
    vrep_common::simRosGetObjectParent::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
*/
bool ROS_server::simRosGetObjectPoseService(
    vrep_common::simRosGetObjectPose::Request& req,
    vrep_common::simRosGetObjectPose::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	float p[3];
	float q[4];
...
bool ROS_server::simRosGetObjectsService(vrep_common::simRosGetObjects::Request&
                                         req, vrep_common::simRosGetObjects::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	res.result = 0;
	res.handles.clear();
	while (true) {
...
bool ROS_server::simRosGetObjectSelectionService(
    vrep_common::simRosGetObjectSelection::Request& req,
    vrep_common::simRosGetObjectSelection::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	int s = simGetObjectSelectionSize();
...
bool ROS_server::simRosGetStringParameterService(
    vrep_common::simRosGetStringParameter::Request& req,
    vrep_common::simRosGetStringParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetStringSignalService(
    vrep_common::simRosGetStringSignal::Request& req,
    vrep_common::simRosGetStringSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	int signalLength;
...
bool ROS_server::simRosGetUIButtonPropertyService(
    vrep_common::simRosGetUIButtonProperty::Request& req,
    vrep_common::simRosGetUIButtonProperty::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetUIEventButtonService(
    vrep_common::simRosGetUIEventButton::Request& req,
    vrep_common::simRosGetUIEventButton::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetUIHandleService(
    vrep_common::simRosGetUIHandle::Request& req,
    vrep_common::simRosGetUIHandle::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetUISliderService(
    vrep_common::simRosGetUISlider::Request& req,
    vrep_common::simRosGetUISlider::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetVisionSensorDepthBufferService(
    vrep_common::simRosGetVisionSensorDepthBuffer::Request& req,
    vrep_common::simRosGetVisionSensorDepthBuffer::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosGetVisionSensorImageService(
    vrep_common::simRosGetVisionSensorImage::Request& req,
    vrep_common::simRosGetVisionSensorImage::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	int resol[2];
	if (simGetVisionSensorResolution(req.handle, resol) != -1) {
		...
		res.image.step = res.image.width *
		                 bytesPerPixel; //Set the image stride in bytes
		...
		if (req.options & 1) {
			...
			for (unsigned int i = 0; i < res.image.height; i++) {
				...
				for (unsigned int j = 0; j < res.image.step; j++)
					res.image.data[msg_idx + j] = (unsigned char)((image_buf[3 *
					                                                         (buf_idx + j) + 0] + image_buf[3 * (buf_idx + j) + 1] + image_buf[3 *
					                                                                 (buf_idx + j) + 2]) / 3);
			}
		} else {
			// Color
...
bool ROS_server::simRosLoadModelService(vrep_common::simRosLoadModel::Request&
                                        req, vrep_common::simRosLoadModel::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	simRemoveObjectFromSelection(sim_handle_all, -1);
	int initValue = simGetBooleanParameter(
...
bool ROS_server::simRosLoadSceneService(vrep_common::simRosLoadScene::Request&
                                        req, vrep_common::simRosLoadScene::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	int initValue = simGetBooleanParameter(
	                    sim_boolparam_scene_and_model_load_messages);
	simSetBooleanParameter(sim_boolparam_scene_and_model_load_messages, 0);
...
bool ROS_server::simRosLoadUIService(vrep_common::simRosLoadUI::Request& req,
                                     vrep_common::simRosLoadUI::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	int handles[1000];
...
bool ROS_server::simRosPauseSimulationService(
    vrep_common::simRosPauseSimulation::Request& req,
    vrep_common::simRosPauseSimulation::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosReadCollisionService(
    vrep_common::simRosReadCollision::Request& req,
    vrep_common::simRosReadCollision::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosReadDistanceService(
    vrep_common::simRosReadDistance::Request& req,
    vrep_common::simRosReadDistance::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosReadForceSensorService(
    vrep_common::simRosReadForceSensor::Request& req,
    vrep_common::simRosReadForceSensor::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	float f[3];
	float t[3];
...
bool ROS_server::simRosReadProximitySensorService(
    vrep_common::simRosReadProximitySensor::Request& req,
    vrep_common::simRosReadProximitySensor::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	float detectedPoint[4];
	res.detectedPoint.resize(3);
	res.normalVector.resize(3);
	res.result = simReadProximitySensor(req.handle, detectedPoint,
	                                    &res.detectedObject, &res.normalVector[0]);
	for (int i = 0; i < 3; i++)
...
bool ROS_server::simRosReadVisionSensorService(
    vrep_common::simRosReadVisionSensor::Request& req,
    vrep_common::simRosReadVisionSensor::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	float* auxValues;
	int* auxValuesCount;
...
bool ROS_server::simRosRemoveObjectService(
    vrep_common::simRosRemoveObject::Request& req,
    vrep_common::simRosRemoveObject::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosRemoveModelService(
    vrep_common::simRosRemoveModel::Request& req,
    vrep_common::simRosRemoveModel::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosRemoveUIService(vrep_common::simRosRemoveUI::Request&
                                       req, vrep_common::simRosRemoveUI::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetArrayParameterService(
    vrep_common::simRosSetArrayParameter::Request& req,
    vrep_common::simRosSetArrayParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetBooleanParameterService(
    vrep_common::simRosSetBooleanParameter::Request& req,
    vrep_common::simRosSetBooleanParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetFloatingParameterService(
    vrep_common::simRosSetFloatingParameter::Request& req,
    vrep_common::simRosSetFloatingParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetFloatSignalService(
    vrep_common::simRosSetFloatSignal::Request& req,
    vrep_common::simRosSetFloatSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetIntegerParameterService(
    vrep_common::simRosSetIntegerParameter::Request& req,
    vrep_common::simRosSetIntegerParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetIntegerSignalService(
    vrep_common::simRosSetIntegerSignal::Request& req,
    vrep_common::simRosSetIntegerSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetJointForceService(
    vrep_common::simRosSetJointForce::Request& req,
    vrep_common::simRosSetJointForce::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetJointPositionService(
    vrep_common::simRosSetJointPosition::Request& req,
    vrep_common::simRosSetJointPosition::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetJointTargetPositionService(
    vrep_common::simRosSetJointTargetPosition::Request& req,
    vrep_common::simRosSetJointTargetPosition::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetJointTargetVelocityService(
    vrep_common::simRosSetJointTargetVelocity::Request& req,
    vrep_common::simRosSetJointTargetVelocity::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetModelPropertyService(
    vrep_common::simRosSetModelProperty::Request& req,
    vrep_common::simRosSetModelProperty::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetObjectFloatParameterService(
    vrep_common::simRosSetObjectFloatParameter::Request& req,
    vrep_common::simRosSetObjectFloatParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	res.result = simSetObjectFloatParameter(req.handle, req.parameter,
	                                        req.parameterValue);
	_handleServiceErrors_end(errorModeSaved);
	return true;
}

bool ROS_server::simRosSetObjectIntParameterService(
    vrep_common::simRosSetObjectIntParameter::Request& req,
    vrep_common::simRosSetObjectIntParameter::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	res.result = simSetObjectIntParameter(req.handle, req.parameter,
	                                      req.parameterValue);
	_handleServiceErrors_end(errorModeSaved);
	return true;
}

bool ROS_server::simRosSetObjectPoseService(
...
bool ROS_server::simRosSetObjectQuaternionService(
    vrep_common::simRosSetObjectQuaternion::Request& req,
    vrep_common::simRosSetObjectQuaternion::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetObjectParentService(
    vrep_common::simRosSetObjectParent::Request& req,
    vrep_common::simRosSetObjectParent::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetObjectPositionService(
    vrep_common::simRosSetObjectPosition::Request& req,
    vrep_common::simRosSetObjectPosition::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetObjectSelectionService(
    vrep_common::simRosSetObjectSelection::Request& req,
    vrep_common::simRosSetObjectSelection::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	simRemoveObjectFromSelection(sim_handle_all, 0);
...
bool ROS_server::simRosGetInfoService(vrep_common::simRosGetInfo::Request& req,
                                      vrep_common::simRosGetInfo::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	int simState = simGetSimulationState();
...
bool ROS_server::simRosSetSphericalJointMatrixService(
    vrep_common::simRosSetSphericalJointMatrix::Request& req,
    vrep_common::simRosSetSphericalJointMatrix::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	res.result = simSetObjectFloatParameter(req.handle, 2013,
	                                        (float)req.quaternion.x);
	if (res.result > 0) {
...
bool ROS_server::simRosSetStringSignalService(
    vrep_common::simRosSetStringSignal::Request& req,
    vrep_common::simRosSetStringSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	res.result = simSetStringSignal(req.signalName.c_str(), req.signalValue.c_str(),
	                                req.signalValue.length());
	_handleServiceErrors_end(errorModeSaved);
	return true;
}

bool ROS_server::simRosAppendStringSignalService(
    vrep_common::simRosAppendStringSignal::Request& req,
    vrep_common::simRosAppendStringSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	...
	res.result = simSetStringSignal(req.signalName.c_str(), theNewString.c_str(),
	                                theNewString.length());
	_handleServiceErrors_end(errorModeSaved);
	return true;
}

bool ROS_server::simRosSetUIButtonLabelService(
    vrep_common::simRosSetUIButtonLabel::Request& req,
    vrep_common::simRosSetUIButtonLabel::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	res.result = simSetUIButtonLabel(req.uiHandle, req.buttonID,
	                                 req.upStateLabel.c_str(), req.downStateLabel.c_str());
	_handleServiceErrors_end(errorModeSaved);
	return true;
}

bool ROS_server::simRosSetUIButtonPropertyService(
    vrep_common::simRosSetUIButtonProperty::Request& req,
    vrep_common::simRosSetUIButtonProperty::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	res.result = simSetUIButtonProperty(req.uiHandle, req.buttonID,
	                                    req.propertyValue);
	_handleServiceErrors_end(errorModeSaved);
	return true;
}

bool ROS_server::simRosSetUISliderService(
    vrep_common::simRosSetUISlider::Request& req,
    vrep_common::simRosSetUISlider::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
...
bool ROS_server::simRosSetVisionSensorImageService(
    vrep_common::simRosSetVisionSensorImage::Request& req,
    vrep_common::simRosSetVisionSensorImage::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	...
	if (result != -1) {
		if (req.image.encoding == sensor_msgs::image_encodings::RGB8) {
			if ((req.image.step == req.image.width * 3) &&
			        (int(req.image.height) == resol[1]) && (int(req.image.width) == resol[0])) {
		...
		if (req.image.encoding == sensor_msgs::image_encodings::MONO8) {
			if ((req.image.step == req.image.width) &&
			        (int(req.image.height) == resol[1]) && (int(req.image.width) == resol[0])) {
				float* image_buff = new float[3 * resol[0]*resol[1]];
				for (unsigned int i = 0; i < req.image.height; i++) {
					int msg_idx = (req.image.height - i - 1) * req.image.step;
...
bool ROS_server::simRosStartSimulationService(
    vrep_common::simRosStartSimulation::Request& req,
...
bool ROS_server::simRosStopSimulationService(
    vrep_common::simRosStopSimulation::Request& req,
...
bool ROS_server::simRosSynchronousService(
    vrep_common::simRosSynchronous::Request& req,
    vrep_common::simRosSynchronous::Response& res) {
	if (req.enable) {
...
bool ROS_server::simRosSynchronousTriggerService(
    vrep_common::simRosSynchronousTrigger::Request& req,
    vrep_common::simRosSynchronousTrigger::Response& res) {
	if (_waitTriggerEnable) {
...
bool ROS_server::simRosTransferFileService(
    vrep_common::simRosTransferFile::Request& req,
    vrep_common::simRosTransferFile::Response& res) {
	res.result = -1;
...
bool ROS_server::simRosEnablePublisherService(
    vrep_common::simRosEnablePublisher::Request& req,
    vrep_common::simRosEnablePublisher::Response& res) {
	int simState = simGetSimulationState();
	if (simState & sim_simulation_advancing)
		res.effectiveTopicName = addPublisher(req.topicName.c_str(), req.queueSize,
		                                      req.streamCmd, req.auxInt1, req.auxInt2, req.auxString.c_str(), 0);
	else
		res.effectiveTopicName = ""; // Error, simulation is not running!
	return true;
}

bool ROS_server::simRosDisablePublisherService(
    vrep_common::simRosDisablePublisher::Request& req,
    vrep_common::simRosDisablePublisher::Response& res) {
	res.referenceCounter = removePublisher(req.topicName.c_str(), false);
	return true;
}


bool ROS_server::simRosEnableSubscriberService(
    vrep_common::simRosEnableSubscriber::Request& req,
    vrep_common::simRosEnableSubscriber::Response& res) {
	int simState = simGetSimulationState();
	if (simState & sim_simulation_advancing)
		res.subscriberID = addSubscriber(req.topicName.c_str(), req.queueSize,
		                                 req.streamCmd, req.auxInt1, req.auxInt2, req.auxString.c_str(), -1, -1);
	else
		res.subscriberID = -1; // Error, simulation is not running!
	return true;
}

bool ROS_server::simRosDisableSubscriberService(
    vrep_common::simRosDisableSubscriber::Request& req,
    vrep_common::simRosDisableSubscriber::Response& res) {
	res.result = removeSubscriber(req.subscriberID);
	return true;
}

bool ROS_server::simRosSetJointStateService(
    vrep_common::simRosSetJointState::Request& req,
    vrep_common::simRosSetJointState::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();

	int setOkCnt = -1; // error
	if ( (req.handles.size() != 0) && (req.handles.size() == req.setModes.size()) &&
	        (req.handles.size() == req.values.size()) ) {
		setOkCnt = 0;
...
bool ROS_server::simRosCreateDummyService(
    vrep_common::simRosCreateDummy::Request& req,
    vrep_common::simRosCreateDummy::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	float* co = NULL;
	float col[12];
...
bool ROS_server::simRosGetAndClearStringSignalService(
    vrep_common::simRosGetAndClearStringSignal::Request& req,
    vrep_common::simRosGetAndClearStringSignal::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	int signalLength;
	char* signalValue = simGetStringSignal(req.signalName.c_str(), &signalLength);
	if (signalValue != NULL) {
...
bool ROS_server::simRosGetObjectGroupDataService(
    vrep_common::simRosGetObjectGroupData::Request& req,
    vrep_common::simRosGetObjectGroupData::Response& res) {
	int errorModeSaved = _handleServiceErrors_start();
	...
	std::vector<float> floatData;
	std::vector<std::string> stringData;
	if (getObjectGroupData(req.objectType, req.dataType, handles, intData,
